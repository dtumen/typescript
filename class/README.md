...
По умолчанию, если нет никаких модификаторов, **то свойство публичное**:
```ts
class Vehicle {
    make: string; // => Неявнр добавляется public make 
}
```
- свойство `make` по умолчанию **публичное**,
- то есть неявно там добавляется **модификатор** `public`:
```ts
class Vehicle {
    public make: string; 
}
...
Виды модификаторов:
1. `public` - свойство/метод доступен из любого места. По умолчанию все свойства и методы публичны.,
2. `private`/`#` - доступ к свойству/методу возможен только внутри класса, 
    - `private` - ограничивает доступ на этапе компиляции, но когда скомпилируется в JS уже не будет приватным.
    - `#` - чтобы обеспечить приватность на уровне языка после компиляции.
3. `protected` - похож на private, но также доступен в **классах-наследниках**.
